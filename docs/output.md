...
???- abstract "Output files"
    ...
    - `polishing/intermediate/cluster/`
        - `<sample_id>/<sample_id>.json`: A json file containign information that is used by viralgenie internally like clusterID, centroid, members, cluster size, if the reference is external or a denovo contig.
...
???- abstract "Output files"
    ...
    - `polishing/`
        ...
        - intermediate/mmseqs/clustered_db/.tsv`: A cluster file containing the clustering information. Where the first column is the cluster representative and the second column the member.
