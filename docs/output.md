...
A visualisation is made to show which regions came from the external reference (red) and which from the denovo contigs (green). For example,

![iVar visualisation example](images/hybrid_consensus_scaffolding.png){: .center}
...
???- abstract "Output files"
    -   `multiqc/`
        -   `overview-tables/`: a directory with a set of commented TSV (comments taken from `--multiqc_comment_headers`) that summarise aspects of the pipeline runs.
