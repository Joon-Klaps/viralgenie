include { FASTQ_FASTA_MAP_CONSENSUS as ITERATION_1 } from '../fastq_fasta_map_consensus'
include { FASTQ_FASTA_MAP_CONSENSUS as ITERATION_2 } from '../fastq_fasta_map_consensus'
include { FASTQ_FASTA_MAP_CONSENSUS as ITERATION_3 } from '../fastq_fasta_map_consensus'
include { FASTQ_FASTA_MAP_CONSENSUS as ITERATION_4 } from '../fastq_fasta_map_consensus'


workflow FASTQ_FASTA_ITERATIVE_CONSENSUS {
    take:
    ch_reference_reads            // channel: [ val(meta), [ reference ], [ fastq ] ]
    ...
    if (repeats >= 2) {
        ch_reference_reads_intermediate = ch_reference_reads_intermediate.map { meta, fasta, reads -> [meta + [iteration: '2', step: "it2", previous_step: meta.step], fasta, reads] }
    ...
    if (repeats >= 3) {
        ch_reference_reads_intermediate = ch_reference_reads_intermediate.map { meta, fasta, reads -> [meta + [iteration: '3', step: "it3", previous_step: meta.step], fasta, reads] }
    ...
    if (repeats >= 4) {
        ch_reference_reads_intermediate = ch_reference_reads_intermediate.map { meta, fasta, reads -> [meta + [iteration: '4', step: "it4", previous_step: meta.step], fasta, reads] }
